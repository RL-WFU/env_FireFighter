...
if __name__ == '__main__':
    ...
    agent_reward_v = [tf.Variable(0, dtype=tf.float32) for i in range(2)]
    # output
    agent_reward_op = [tf.summary.scalar('agent' + str(i) + '_reward', agent_reward_v[i]) for i in range(2)]


    # list of agent's action
    agent_a1 = [tf.Variable(0, dtype=tf.float32) for i in range(2)]

    agent_a1_op = [tf.summary.scalar('agent' + str(i) + '_action_1', agent_a1[i]) for i in range(2)]


    reward_100 = [tf.Variable(0, dtype=tf.float32) for i in range(3)]

    reward_100_op = [tf.summary.scalar('agent' + str(i) + '_reward_l100_mean', reward_100[i]) for i in range(2)]



    reward_1000 = [tf.Variable(0, dtype=tf.float32) for i in range(3)]

    reward_1000_op = [tf.summary.scalar('agent' + str(i) + '_reward_l1000_mean', reward_1000[i]) for i in range(2)]
    ...
    agent1_memory = ReplayBuffer(100)

    agent2_memory = ReplayBuffer(100)
    ...
    # make graph with reward
        ...
        if (i+1) % 100 == 0:
            reward_each_episode.append(glob_reward)
        ...
        agent1_memory.add(np.vstack([o_n[0], o_n[1]]),

                          np.vstack([agent1_action, agent2_action]),

                          r_n[0], np.vstack([o_n_next[0], o_n_next[1]]), False)



        agent2_memory.add(np.vstack([o_n[1], o_n[2], o_n[0]]),

                          np.vstack([agent2_action,agent1_action]),

                          r_n[1], np.vstack([o_n_next[1], o_n_next[0]]), False)
    ...
    sess.close()
    plt.plot(x, reward_each_episode)
    plt.show()